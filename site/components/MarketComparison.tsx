    ...
    <section className="py-[clamp(48px,6vw,80px)] px-[clamp(24px,5vw,80px)]">
      <FadeIn className="max-w-[1100px] mx-auto">
        <div className="text-center mb-[clamp(32px,5vw,48px)]">
          ...
          <p className="text-[15px] font-light leading-relaxed text-ink-light max-w-[640px] mx-auto">
        ...
        <div className="flex items-center justify-center gap-4 px-8 py-5 bg-white border border-border-warm rounded-xl mb-[clamp(32px,4vw,48px)] max-w-[600px] mx-auto">
          ...
          <span className="text-[14px] font-light text-ink-light leading-snug text-left">
            of Thai bottled water is <strong className="font-semibold text-ink">filtered, not mineralized</strong>.<br />
              ...
              <div className="space-y-3">
                {['Limited shelf presence in pharmacies', 'Still dominated by sugary sports drinks', 'Early stage = massive first-mover opportunity'].map((item, i) => (
                  <div key={item} className="flex items-start gap-2.5">
                    ...
                    <span className={`text-[13px] font-light ${i === 2 ? 'text-golden-hour font-medium' : 'text-white/70'}`}>{item}</span>
                  </div>
                ))}
              </div>
        ...
        <FadeIn className="text-center mt-8">
          <p className="text-[16px] font-display italic text-mineral">
            The wave is coming.
          </p>
        </FadeIn>
      </FadeIn>
